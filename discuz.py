...
class DiscuzScan(object):
    ...
    def GetVersion(self,target):
        ...
        VERSION=re.findall('(X2)|(X2.5)|(X3.1)|(X3.2)|(X3)|(7.0.0)|(7.2)|(6.0.0)',str(result[2]))
    ...
    def PwdList(self,target):
        ...
        result=self.Scanner.GetData(target+'utility/convert/data/config.inc.php')
        if result[1]==200:
            colprint.strprint(10,'utility path is found.')
        result=self.Scanner.GetData(target+'jiaoyou.php')
        if result[1]==200:
            colprint.strprint(10,'jiaoyou path is found.')
        ...
        with open('password.txt','r') as pwdlist:
            for line in  pwdlist:
                ...
                if re.findall('(sid=)',result[0]):
                    colprint.strprint(10,'Founder Passwd is Found '+password)
